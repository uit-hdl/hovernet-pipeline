...
def run_nuclei_type_stat(pred_dir, true_dir, nuclei_type_dict, type_uid_list=None, exhaustive=True, rad=12):
    '''
    rad = 12 if x40
    rad = 6 if x20
    '''
    ...
    for file_idx, filename in enumerate(file_list[:]):
        ...
        paired, unpaired_true, unpaired_pred = pair_coordinates(true_centroid, pred_centroid, rad)
    ...
    def _f1_type(paired_true, paired_pred, unpaired_true, unpaired_pred, type_id, w):
        ...
        new_f1 = 2 * (precision_type * recall_type) / (precision_type + recall_type) # just check, same as f1_type
    ...
    for type_uid in type_uid_list:
        f1_type, precision_type, recall_type, new_f1 = _f1_type(paired_true_type, paired_pred_type, 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--map', help="Name of the nuclei type mapping", choices=['consep','pannuke'], required=True)
    parser.add_argument('--type', help="Run type stats", default=False, action='store_true')
    parser.add_argument('--inst', help="Run inst stats", default=False, action='store_true')

    args = parser.parse_args()
    if args.type:
        print ("---Type statistics---")
        run_nuclei_type_stat(args.pred_dir, args.true_dir, MAP_TYPES[f'nuclei_types_hv_{args.map}'])
    if args.inst:
