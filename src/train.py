    ...
    def get_datagen(self, batch_size, mode="train", view=False):
        if mode == "train":
            augmentors = self.get_train_augmentors(
                self.train_input_shape, self.train_mask_shape, view
            )
            data_files = get_files(self.train_dir, self.data_ext)
            print(
                f"Train data length: {len(data_files)} in <{self.train_dir}>.{self.data_ext}"
            )
            data_generator = loader.train_generator
            nr_procs = self.nr_procs_train
        else:
            augmentors = self.get_valid_augmentors(
                self.infer_input_shape, self.infer_mask_shape, view
            )
            data_files = get_files(self.valid_dir, self.data_ext)
            print(
                f"Valid data length: {len(data_files)} in <{self.valid_dir}>.{self.data_ext}"
            )
...
if __name__ == "__main__":
    ...
    if args.view:
        ...
        with open("config.yml", "r") as source:
            with open(
                os.path.join(trainer.save_dir, "config.yml"), "w"
            ) as train_config:
