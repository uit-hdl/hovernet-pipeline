...
class InfererExternal:
    ...
    def __gen_prediction(self, x, predictor, compact=None):
        ...
        while len(sub_patches) > self.inf_batch_size:
            ...
            mini_output = (
                predictor(mini_batch)[0] if compact == False else predictor(mini_batch)
            )
            mini_output = np.split(mini_output, self.inf_batch_size, axis=0)
            pred_map.extend(mini_output)
        if len(sub_patches) != 0:
            mini_output = (
                predictor(sub_patches)[0]
                if compact == False
                else predictor(sub_patches)
            )
    ...
    def apply_compact(self, prefix="import/"):
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            with tf.gfile.GFile(self.inf_model_path, "rb") as f:
                ...
                input_img = sess.graph.get_tensor_by_name(
                    "{}{}".format(prefix, self.eval_inf_input_tensor_names[0])
                )
                prediction_img = sess.graph.get_tensor_by_name(
                    "{}{}".format(prefix, self.eval_inf_output_tensor_names[0])
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path", help="Path to the model pb/checkpoint", required=True
    )
    parser.add_argument("--input_img", help="Full path to input image", required=True)
    parser.add_argument(
        "--save_dir", help="Path to the directory to save .mat result", required=True
    )
